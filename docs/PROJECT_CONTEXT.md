...
| 7 | **5 — Issues & concurrency** | Issue inheritance (Epic/Story/Task/Bug), parent-child self-reference, labels, **watchers** (add/remove, auto-watch on assign & comment), issue-key generation `TF-101` safe under concurrency, `@Version` → HTTP 409, comments with ownership rules | Inheritance strategies + query cost, self-references, optimistic vs pessimistic locking and *why each is used where*, entity `equals`/`hashCode` |
...
| 10 | **9 (trimmed) — Events & async** | Application events on issue changes, `@TransactionalEventListener(AFTER_COMMIT)`, `@Async` notifications, in-app notifications, **@mention parsing in comments** → notify mentioned project members, dev vs prod email sender, one `@Scheduled` job | Phantom-notification trap, custom `TaskExecutor` with bounded queue, propagating security context to async threads, idempotent scheduled jobs |
...
⚠️ **Cross-phase dependency:** Phase 9 notifications ("mentioned", "comment on a watched issue") require the **watcher model from Phase 5** to already exist. If Phase 5 gets trimmed, watchers are not the thing to cut.
